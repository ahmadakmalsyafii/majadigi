...
class HargaBahanPokokBloc extends Bloc<HargaBahanPokokEvent, HargaBahanPokokState> {
  ...
  Future<void> _onFetchCommodityList(FetchCommodityList event, Emitter<HargaBahanPokokState> emit) async {
    ...
    result.fold(
      (failure) => emit(CommodityListError(failure.message)),
      (response) {
        var items = response.data.priceList;
        if (event.searchQuery.isNotEmpty) {
          items = items.where((item) => item.commodityName.toLowerCase().contains(event.searchQuery.toLowerCase())).toList();
        }
  ...
  Future<void> _onFetchCommodityDetail(FetchCommodityDetail event, Emitter<HargaBahanPokokState> emit) async {
    ...
    cityResult.fold(
      ...
      (cityResponse) => emit(CommodityDetailLoaded(detailData, cityResponse.data)),
    );
  }
}
